...
const App = () => {
  ...
  const checkStatusHandler = async () => {
    try {
      await assembly.get(`/transcript/${transcriptID}`).then((res) => {
		...
		setSentiment(transcriptData.text)
      })
    } catch (err) {
      console.error(err)
    }
  ...
  const sentimentAnalysier = async () = {
    try {
      await assembly.get(`/transcript/${transcriptID}`).then((res) => {
        setTranscriptData(res.data)
		setTranscript(transcriptData.sentiment_analysis_results)
      })
    } catch (err) {
      console.error(err)
    }
  ...
  return (
    <div className="Contanier">
      ...
      <h3 className="Text2">{transcriptData.sentiment_analysis_results}</h3>
    </div>  
  )
}
